...
public void testOrgTwodeeSpeccheckExampleInPackage() throws Exception {
  ...
  Method method = null;
  try {
    method = Class.forName("org.twodee.speccheck.example.InPackage").getDeclaredMethod("foo", new Class<?>[]{org.twodee.speccheck.example.InPackageTypeA.class, org.twodee.speccheck.example.InPackageTypeB.class});
  } catch (NoSuchMethodException e) {
    Assert.fail("You need a foo method in class org.twodee.speccheck.example.InPackage taking 2 arguments, having types org.twodee.speccheck.example.InPackageTypeA, org.twodee.speccheck.example.InPackageTypeB.");
  }
  Assert.assertEquals("Your method foo(org.twodee.speccheck.example.InPackageTypeA, org.twodee.speccheck.example.InPackageTypeB) in class org.twodee.speccheck.example.InPackage has the wrong return type.", void.class, method.getReturnType());
  Assert.assertTrue("The modifiers for method foo(org.twodee.speccheck.example.InPackageTypeA, org.twodee.speccheck.example.InPackageTypeB) in class org.twodee.speccheck.example.InPackage are not correct. " + SpecCheckUtilities.getModifierDifference(9, method.getModifiers()), 9 == method.getModifiers());
  exceptions = java.util.Arrays.asList(method.getExceptionTypes());
  outlawedExceptions = java.util.Arrays.asList(new Class<?>[]{});
  for (Class<?> exception : exceptions) {
    Assert.assertFalse("The specification requires method foo(org.twodee.speccheck.example.InPackageTypeA, org.twodee.speccheck.example.InPackageTypeB) in class org.twodee.speccheck.example.InPackage to handle and not throw " + exception.getName() + ".", outlawedExceptions.contains(exception));
  }
...
public void testOrgTwodeeSpeccheckExampleInPackageTypeB() throws Exception {
  ...
  try {
     Class<?> cls = Class.forName("org.twodee.speccheck.example.InPackageTypeB");
     Assert.assertEquals("The superclass of class org.twodee.speccheck.example.InPackageTypeB is not correct.", org.twodee.speccheck.example.InPackageTypeA.class, cls.getSuperclass());
  } catch (ClassNotFoundException e) {
     Assert.fail("A class by the name of org.twodee.speccheck.example.InPackageTypeB could not be found. Check case, spelling, and that you created your class in the right package.");
  } catch (NoClassDefFoundError e) {
     Assert.fail("A class by the name of org.twodee.speccheck.example.InPackageTypeB could not be found. Check case, spelling, and that you created your class in the right package.");
