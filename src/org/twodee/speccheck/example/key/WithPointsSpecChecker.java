...
public class WithPointsSpecChecker {
  ...
  public void testOrgTwodeeSpeccheckExampleWithPoints() throws Exception {
    try {
      ...
      Assert.assertTrue("The modifiers for class org.twodee.speccheck.example.WithPoints are not correct. " + SpecCheckUtilities.getModifierDiff(1, cls.getModifiers()), 1 == cls.getModifiers());
    } catch (ClassNotFoundException e) {
      Assert.fail("A class by the name of org.twodee.speccheck.example.WithPoints could not be found. Check case, spelling, and that you created your class in the right package.");
    } catch (NoClassDefFoundError e) {
      Assert.fail("A class by the name of org.twodee.speccheck.example.WithPoints could not be found. Check case, spelling, and that you created your class in the right package.");
    ...
    Assert.assertTrue("The modifiers for constructor org.twodee.speccheck.example.key.WithPoints(java.lang.String) are not correct. " + SpecCheckUtilities.getModifierDiff(1, ctor.getModifiers()), 1 == ctor.getModifiers());
    ...
    Assert.assertTrue("The modifiers for method getFive() in class org.twodee.speccheck.example.WithPoints are not correct. " + SpecCheckUtilities.getModifierDiff(1, method.getModifiers()), 1 == method.getModifiers());
